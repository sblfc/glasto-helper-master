                ...
                if (i != await this.getHighestScoringTabIndex() || await this.tabs[i].getSimilarityScore() == -1) {
                    if (await this.tabs[i].getReady() == true) {
                        ...
                        if (finishTime - this.tabs[i].getStartTime() < this.refreshRateInMs) {
                            await this.sleep((this.refreshRateInMs - (finishTime - this.tabs[i].getStartTime())) + Math.random());
                        }
                    } else {
...
class Tab {
    ...
    async loadPage() {
        ...
        this.page.on('request', async req => {
            ['stylesheet', 'font', 'image'].includes(req.resourceType()) ? await req.abort() : await req.continue();
        });
